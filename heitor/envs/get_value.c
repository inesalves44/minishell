...
/*   Updated: 2023/04/04 23:20:29 by hmaciel-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	refresh_env_array(t_root *root)
{
	...
	while (root->env_lst)
	{
		...
		root->env_array[i] = ft_strjoin_gnl(root->env_array[i], root->env_lst->key);
		root->env_array[i] = ft_strjoin_gnl(root->env_array[i], "=");
		root->env_array[i] = ft_strjoin_gnl(root->env_array[i], root->env_lst->value);
		i++;
...
char    *get_env_value(t_root *root, char *key)
{
	...
	while(root->env_lst)
	{
		...
		if (ft_strncmp(key, root->env_lst->key, ft_strlen(key)) == 0)
		{
...
int    print_env_value(t_root *root, char *key)
{
	...
	while(root->env_lst)
	{
		if (ft_strncmp(key, root->env_lst->key, ft_strlen(root->env_lst->key)) == 0 &&
			ft_strlen(key) == ft_strlen(root->env_lst->key))
		{
