...
/*   Updated: 2023/04/28 18:49:29 by hmaciel-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	**creatingvalue(char **split)
{
	...
	if (i % 2 == 0)
		...
		len = len + i - 1;
	i = 0;
	split2 = ft_calloc(sizeof(char *), len + 1); // + 1 to corret the invalid read of size!
...
char	*command_expander_2(t_root *r, int i)
{
	...
	j = 0;
	temp = NULL;
	test = NULL; // NULL becouse now i used join_gnl
	//test = ft_strdup(""); //
	while (split[j])
	{
		...
		if (temp)
		{
			test = ft_strjoin_gnl(test, temp); // changed to ft_strjoin_gnl to accept null.
			free(temp);
			temp = NULL;
		}
		j++;
